...
void DAGQueryBlockInterpreter::handleTableScan(const TiDBTableScan & table_scan, DAGPipeline & pipeline)
{
    ...
    DAGStorageInterpreter storage_interpreter(context, table_scan, push_down_filter, max_streams);  // 构造函数，变量名就是 storage_interpreter
...
void DAGQueryBlockInterpreter::handleJoin(const tipb::Join & join, DAGPipeline & pipeline, SubqueryForSet & right_query)
{
    ...
    size_t join_build_concurrency = settings.join_concurrent_build ? std::min(max_streams, build_pipeline.streams.size()) : 1;  // 看下 build 端的 stream 并发数量
...
void DAGQueryBlockInterpreter::handleExchangeReceiver(DAGPipeline & pipeline)
{
    ...
    for (size_t i = 0; i < stream_count; ++i)
    {
        ...
        exchange_receiver_io_input_streams.push_back(stream);    // 构建 input stream 往 set 里面加
...
BlockInputStreams DAGQueryBlockInterpreter::execute()
{
    ...
    return pipeline.streams;   // 直接返回这些被装饰过的流
}
} // namespace DB
